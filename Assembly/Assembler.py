...
class Assembler():
    labels = {}
    jpoint_instrs = {}
    RType = ['ADD', 'ADDU', 'ADDC', 'ADDCU',
             'SUB', 'CMP', 'CMPU', 'AND', 'OR', 'XOR']
    Immediates = ['ADDI', 'ADDUI', 'ADDCI', 'ADDCUI',
                  'SUBI', 'CMPI', 'CMPUI', 'ANDI', 'ORI', 'XORI']
    Shift = ['LSH', 'RSH', 'ALSH', 'ARSH']
    ImmdShift = ['LSHI', 'RSHI', 'ALSHI', 'ARSHI']
    Branch = ['BEQ', 'BNE', 'BGE', 'BCS', 'BCC', 'BHI', 'BLS',
              'BLO', 'BHS', 'BGT', 'BLE', 'BFS', 'BFC', 'BLT', 'BUC']
    Jump = ['JEQ', 'JNE', 'JGE', 'JCS', 'JCC', 'JHI', 'JLS',
            'JLO', 'JHS', 'JGT', 'JLE', 'JFS', 'JFC', 'JLT', 'JUC']
        ...
        for x in f:
            ...
            if ((len(parts) > 0) and (line[0] != '.')):
                ...
                if (instr in self.RType):
                    if (len(parts) == 2):
                        ...
                        if ((firstReg in self.REGISTERS) and (secondReg in self.REGISTERS)):
                            firstRegNum = '{0:04b}'.format(
                                int(firstReg.replace('%r', '')))
                            secondRegNum = '{0:04b}'.format(
                                int(secondReg.replace('%r', '')))
                            data = '0000' + secondRegNum + \
                                self.instrCode(instr) + firstRegNum
                            wf.write(data + '\n')
                        else:
                            sys.exit(
                                'Syntax Error: R-type needs two registers')
                    else:
                    ...
                    if (len(parts) == 2):
                        ...
                        if ((Immd[0] in '$') and (secondReg in self.REGISTERS)):
                            ...
                            if ((immdInt > 127) or (-128 > immdInt)):
                                ...
                                sys.exit(
                                    'Syntax Error: Immediate can not be larger then 127 or less then -128, got ' + str(immdInt))
                            elif (immdInt >= 0):
                                immediate = '{0:08b}'.format(immdInt)
                            else:
                                immediate = '{0:08b}'.format(
                                    ((-1 * immdInt) ^ 255) + 1)
                            secondRegNum = '{0:04b}'.format(
                                int(secondReg.replace('%r', '')))
                            data = self.instrCode(
                                instr) + secondRegNum + immediate
                            wf.write(data + '\n')
                        else:
                            sys.exit(
                                'Syntax Error: Immediate operations need an immd then a register')
                    else:
                        sys.exit(
                            'Syntax Error: Immediate operations need two args: ' + line)
                    ...
                    if (len(parts) == 2):
                        ...
                        if ((firstReg in self.REGISTERS) and (secondReg in self.REGISTERS)):
                            ...
                            secondRegNum = '{0:04b}'.format(
                                int(secondReg.replace('%r', '')))
                            data = '1000' + secondRegNum + \
                                self.instrCode(instr) + firstRegNum
                            wf.write(data + '\n')
                        else:
                            sys.exit(
                                'Syntax Error: shifts needs two self.REGISTERS')
                    else:
                    ...
                    if (len(parts) == 2):
                        ...
                        if ((Immd[0] in '$') and (secondReg in self.REGISTERS)):
                            ...
                            if ((immdInt > 15) or (0 > immdInt)):
                                sys.exit(
                                    'Syntax Error: Immediate can not be larger then 15 or less then 0')
                            else:
                                immediate = '{0:04b}'.format(immdInt)
                            secondRegNum = '{0:04b}'.format(
                                int(secondReg.replace('%r', '')))
                            data = '1000' + secondRegNum + \
                                self.instrCode(instr) + immediate
                            wf.write(data + '\n')
                        else:
                            sys.exit(
                                'Syntax Error: Immediate shifts need an immd then a register')
                    else:
                    ...
                    if (len(parts) == 1):
                        ...
                        if (Disp[0] in '$'):
                            ...
                            if ((dispInt > 255) or (-255 > dispInt)):
                                sys.exit(
                                    'Syntax Error: Branch can not be larger then 255 or less then -255')
                            elif (dispInt >= 0):
                                Displacement = '{0:08b}'.format(dispInt)
                            else:
                                Displacement = '{0:08b}'.format(
                                    ((-1 * dispInt) ^ 255) + 1)
                            data = '1110' + \
                                self.instrCode(instr.replace(
                                    'B', '')) + Displacement
                            wf.write(data + '\n')
                        elif (Disp[0] == '.'):
                            ...
                            if ((dispInt > 255) or (-255 > dispInt)):
                                ...
                                Displacement = '{0:08b}'.format(
                                    ((-1 * dispInt) ^ 255) + 1)
                            data = '1110' + \
                                instrCode(instr.replace('B', '')) + \
                                Displacement
                            wf.write(data + '\n')
                        else:
                            sys.exit(
                                'Syntax Error: Branch operations need a displacement or label')
                    else:
                    ...
                    if (len(parts) == 1):
                        ...
                        if ((firstReg in self.REGISTERS)):
                            ...
                            sys.exit(
                                'Syntax Error: Jump operations need a register')
                    else:
                    ...
                    if (len(parts) == 1):
                        ...
                        if ((firstReg in self.REGISTERS)):
                            ...
                            sys.exit(
                                'Syntax Error: NOT operation needs one register')
                    else:
                ...
                # pseudo-instruction to point to jump addresses
                    ...
                    if (len(parts) != 2):
                        sys.exit(
                            'Syntax Error: JPT needs a pointer and a register')
                    pointer = parts.pop(0)
                    reg = parts.pop(0)
                    if pointer not in self.jpoint_instrs:
                    ...
                    shift = '1000' + regNumber + \
                        self.instrCode('LSHI') + shift_amt
